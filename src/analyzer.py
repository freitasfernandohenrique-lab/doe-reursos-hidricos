"""Agregações e métricas para resumo diário e janela móvel."""
...
def analyze(items_1d: list[MatchItem], today_iso: str) -> dict[str, Any]:
    all_sorted = _sort_items(items_1d)
    today_items = [i for i in all_sorted if i.date_iso == today_iso]

    keyword_counter = Counter()
    theme_counter = Counter()
    link_counter = Counter(i.link for i in items_1d)
    ...
    return {
        ...
        "counts": {
            ...
            "items_1d": len(items_1d),
            "new_items": new_items,
            "recurring_items": recurring,
        },
        "top_day": [i.to_dict() for i in top_day],
        "today_items": [i.to_dict() for i in today_items],
        "top_1d": [i.to_dict() for i in top_1d],
        "keywords_1d": dict(keyword_counter.most_common()),
        "themes_1d": dict(theme_counter.most_common()),
    }
